...
import pandas as pd
import yaml
from STL_module import calculate_all_stds,plot_stds,calculate_std_with_stl,calculate_all_means,plot_mu
...
if __name__ == '__main__':
    with open(sys.argv[1], "r",encoding="utf-8") as ymlfile:
        ...
        if data_yml.get("error_range", False):
            # Cargar datos de producción y demanda
            prod_by_day_df = pd.read_csv(data_yml["data"], parse_dates=[data_yml["date"]]).set_index('Date').asfreq('D')
            demand_df = pd.read_csv('data/Demand.csv', parse_dates=['Date']).set_index('Date').asfreq('D')
            
            # Calcular todas las desviaciones estándar y generar el archivo CSV
            all_stds_df = calculate_all_stds(prod_by_day_df, demand_df, 'output/desviaciones_estandar.csv')
            all_mu_df=calculate_all_means(prod_by_day_df, demand_df,  'output/all_mean.csv')
            # Graficar las desviaciones estándar y guardar la imagen
            plot_stds(all_stds_df, 'output/desviaciones_estandar.png')
            plot_mu(all_mu_df, 'output/all_mean.png')